...
describe Value do
  ...
  it 'errors if you instantiate it from a hash with unrecognised fields' do
    expect { Money.with(:unrecognized_field => 1, :amount => 2, :denomination => 'USD') }.to raise_error { |error|
      expect(error).to be_a(Values::FieldError)
      expect(error.missing_fields).to be_empty
      expect(error.unexpected_fields).to eq %I[unrecognized_field]
    }
  end

  it 'errors if you instantiate it from a hash with missing fields' do
    expect { Money.with({}) }.to raise_error { |error|
      expect(error).to be_a(Values::FieldError)
      expect(error.missing_fields).to eq %I[amount denomination]
      expect(error.unexpected_fields).to be_empty
    }
  end
  ...
  describe '#with' do
    ...
    describe 'with hash arguments' do
      ...
      it 'raises argument error if unknown field' do
        expect { p.with({ :foo => 3 , :bar => "baz" }) }.to raise_error { |error|
          expect(error).to be_a(Values::FieldError)
          expect(error.unexpected_fields).to eq %I[foo bar]
          expect(error.missing_fields).to be_empty
        }
      end
    end
